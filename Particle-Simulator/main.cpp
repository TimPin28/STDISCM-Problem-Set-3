...
boolean hasParticles = false; // Flag to check if there are particles in the simulation
...
void updateParticleWorker(std::vector<Particle>& particles, double deltaTime, double simWidth, double simHeight, SOCKET serverSocket) {
    while (!done) {
        ...
        while (true) {
            ...
            // print particle
            std::cout << "Particle " << index << " x: " << particles[index].x << " y: " << particles[index].y << std::endl;
        }
    }
}
